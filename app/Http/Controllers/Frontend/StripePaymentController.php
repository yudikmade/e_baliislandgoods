    ...
    public function indexGiftCard(Request $request){
        ...
        if($validator->fails()) {
            ...
            if(isset($getPrice->product_id)){
                ...
                $customer_email = $input['emailGiftCard'];
                ...
                if($doNext){
                    $email = $input['emailGiftCard'];
                    $coupon = 'bcwf-'.$charge['id'];
                    ...
                    $dataCoupon->discount = $getPrice->price;
                    $dataCoupon->status = 1;
                    ...
                    $dataSave->total_price = $getPrice->price;
                    $dataSave->shipping_cost = 0;
                    $dataSave->additional_price = 0;
                    $dataSave->tax = 0;
                    $dataSave->total_payment = $getPrice->price;
                    $dataSave->type_payment = 'stripe';
                    ...
                    if($dataSave->save()){
                        ...
                        $dataDetail->qty = 1;
                        $dataDetail->price = $getPrice->price;
                        $dataDetail->status = 1;
                        $dataDetail->save();
                        ...
                        $giftCardDefault = Common_helper::convert_to_current_currency($getPrice->price);
                    ...
                    $result['trigger'] = 'yes';
                    $result['notif'] = 'Payment has been successful, please wait until the process is complete.';
                    $result['direct'] = route('user_success_payment_gift_card_stripe');
                }
            }
        }
